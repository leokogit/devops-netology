...
<details><summary>Спойлер, Задача 4 </summary>
...
<details><summary>Спойлер, Задача 5 </summary>
...
---
### 6. Настроить CI/CD для автоматического развёртывания приложения. (Установка Gitlab CE и Gitlab Runner) .  Сделано
<details><summary>Спойлер, Задача 6 </summary>
    
+   Настроена CI/CD система для автоматического развертывания приложения при изменении кода.  Сделано   
+   Построен pipeline доставки кода в среду эксплуатации, то есть настроить автоматический деплой на сервер `app.you.domain` при коммите в репозиторий с WordPress.  Сделано  
...
Результаты: 
+  Созданы роли для установки и первичной настройки Gitlab CE и Gitlab runner, также отдельная роль для создания и копирования ssh ключей на нужные инстансы, создания пользователя (для раннера). 
+  В Gitlab Создан пользователь, группа, проект, настроен pipeline. 
+  Зарегистрирован Gitlab runner для созданного проекта.
+  При коммите в репозиторий успешно отрабатываются jobs , происходит деплой на веб сервер с wordpress (app.domain)

<p align="center"> <img src="./assets/jobs.jpg"></p>


---
### 7. Настроить мониторинг инфраструктуры с помощью стека: Prometheus, Alert Manager и Grafana. Сделано
<details><summary>Спойлер, Задача 7 </summary>
    
+   Разработана Ansible роль для установки Prometheus, Alert Manager и Grafana. Cltkfyj      
...
Предварительно , отдельной ролью настраиваются ssh ключи, подготваливается среда для gitlab и runner, создаются пользователи и тд. (мне удобнее былло отдельно контролировать этот процесс в отдельной роли, но можно конечно всё в одной запускать, но на первых этапах сложнее было бы отлавливать все ошибки в гигантской "портянке" из задач )      

---
## Итоговый результат, скриншоты сервисов и ссылки на конфиги 

<details><summary>Спойлер, описание задачи итоговых результатов </summary>
...
Результаты:      
ps. Виртуалки выключены для экономии средств, если потребуется продемонстрировать, то включю..        
